...
Provides:
    ...
    - ``MultiTaskNILModel``: ``NILTransformerEncoder`` + both heads.
...
class MultiTaskHead(nn.Module):
    ...
    def __init__(self, input_dim: int = 128, num_tiers: int = 5,
                 alpha: float = 0.5) -> None:
        super().__init__()
        self.alpha = alpha
        self.tier_head = NILTierClassificationHead(
            input_dim=input_dim, num_tiers=num_tiers
        )
        self.value_head = NILValuationRegressionHead(input_dim=input_dim)
